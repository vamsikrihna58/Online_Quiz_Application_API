...
public class Online_Quiz_Service {
	...
	public Quiz_Management save_Quiz_Info(Quiz_Management quiz_management) { // to create the quiz

		return quiz_info.save(quiz_management);

	}

	public Questions add_Questions(int quiz_id, Questions questions) {  //to add quesions in specifice quiz
		Quiz_Management byId = quiz_info.findById(quiz_id).orElseThrow(() -> new Quiz_Not_Found(quiz_id));
		

		questions.setQuiz_management(byId);
		long correctCount = questions.getOptionsEntities().stream()
	            .filter(OptionsEntity::isCorrect_Or_Not)
	            .count();
	    
	    if (correctCount != 1) {
	        throw new RuntimeException("A question must have exactly one correct option.");
	    }
	...
	public List<getAllQuesionsByEntity> get_All_Questions(int quiz_id) {  // to get the all questions based on specific quiz
		Quiz_Management byId = quiz_info.findById(quiz_id).orElseThrow(() -> new Quiz_Not_Found(quiz_id));
	...
	public SumbitionResponse submit(int quiz_id, List<SubmitionItems> items) { // to submit the answer by specific quiz
		Quiz_Management byId = quiz_info.findById(quiz_id).orElseThrow(() -> new Quiz_Not_Found(quiz_id));
	...
	public List<Quiz_Management> get_All_Quiz(){ //to get all quizzes
		return quiz_info.findAll();
	}
